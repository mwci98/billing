...
async function resolveLabel(fragment: string, response: any) {
  ...
  try {
    ...
    const cleaned = suggestion.replace(/\b(mobile|smartphone|phone|model)\b/gi, '').replace(/\s+/g, ' ').trim();
    response.setHeader('Cache-Control', 's-maxage=86400, stale-while-revalidate=604800');
    return response.status(200).json({
      found: true,
      name: cleaned.replace(/\b\w/g, (letter) => letter.toUpperCase()),
...
export default async function handler(request: any, response: any) {
  ...
  const labelText = String(request.query?.label || '').trim().toLowerCase();
  if (labelText) return resolveLabel(labelText, response);
