      ...
      <div className="grid gap-5 xl:grid-cols-[minmax(0,1fr)_390px]">
        <section className="space-y-4">
          ...
          <div className="rounded-3xl border border-gray-200 bg-white p-4 dark:border-white/10 dark:bg-[#141416] sm:p-5">
            ...
            {menuProducts.length ? <div className="mt-5 grid grid-cols-2 gap-3 md:grid-cols-3 2xl:grid-cols-4">{menuProducts.map(product => <button key={product.id} onClick={() => addItem(product)} className="min-h-32 rounded-2xl border border-gray-200 p-4 text-left transition hover:-translate-y-0.5 hover:border-emerald-500/40 hover:shadow-lg dark:border-white/10 dark:bg-white/[0.025]"><span className="flex h-9 w-9 items-center justify-center rounded-xl bg-emerald-500/10 text-emerald-500"><ChefHat className="h-4 w-4" /></span><p className="mt-4 line-clamp-2 text-sm font-black">{product.name}</p><div className="mt-2 flex items-center justify-between gap-2"><span className="truncate text-[10px] font-bold uppercase text-gray-400">{product.menuVariants?.length ? `${product.menuVariants.length} choices` : product.category}</span><span className="font-mono text-sm font-black text-emerald-500">{product.menuVariants?.length ? 'From ' : ''}{settings.currency}{Math.min(product.sellingPrice, ...(product.menuVariants || []).map(variant => variant.price)).toFixed(2)}</span></div></button>)}</div> : <div className="flex min-h-64 flex-col items-center justify-center text-center text-gray-500"><ChefHat className="mb-3 h-8 w-8 opacity-40" /><p className="font-bold">No menu items found</p><p className="mt-1 text-xs">Add dishes in Menu Items or change the filter.</p></div>}
          </div>
        </section>
