...
export const RestaurantBilling: React.FC = () => {
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="grid gap-5 sm:grid-cols-[minmax(0,1fr)_280px] md:grid-cols-[minmax(0,1fr)_300px] lg:grid-cols-[minmax(0,1fr)_360px] xl:grid-cols-[minmax(0,1fr)_390px]">
        <section className="min-w-0 space-y-4">
          {orderType === 'Dine In' && <div className="grid grid-cols-2 gap-2 rounded-3xl border border-gray-200 bg-white p-3 dark:border-white/10 dark:bg-[#141416] sm:gap-3 sm:p-4"><Field icon={Armchair} label="Table"><OrderCounter value={Math.max(1, Number(tableNumber) || 1)} onChange={value => setTableNumber(String(value))} label="table number" /></Field><Field icon={UsersRound} label="Guests"><OrderCounter value={guestCount} onChange={setGuestCount} label="guest count" /></Field></div>}
          <div className="rounded-3xl border border-gray-200 bg-white p-4 dark:border-white/10 dark:bg-[#141416] sm:p-5">
            ...
            {menuProducts.length ? <div className={`mt-4 min-w-0 gap-2.5 ${menuView === 'compact' ? 'max-h-[24rem] space-y-2 overflow-y-auto pr-1 sm:grid sm:max-h-none sm:grid-cols-1 sm:space-y-0 sm:gap-3 sm:overflow-visible sm:pr-0' : 'touch-scroll flex overflow-x-auto pb-2 sm:grid sm:grid-cols-2 sm:gap-3 sm:overflow-visible sm:pb-0 lg:grid-cols-3 2xl:grid-cols-4'}`}>{menuProducts.map(product => <button key={product.id} onClick={() => addItem(product)} className={`group min-w-0 overflow-hidden rounded-2xl border border-gray-200 text-left transition hover:-translate-y-0.5 hover:border-emerald-500/40 hover:shadow-lg dark:border-white/10 dark:bg-white/[0.025] ${menuView === 'compact' ? 'flex min-h-14 items-center sm:min-h-[4.5rem]' : 'w-[calc((100vw-5rem)/3)] min-w-[6.5rem] shrink-0 sm:w-auto sm:min-w-0'}`}><div className={`flex items-center justify-center overflow-hidden bg-emerald-500/5 ${menuView === 'compact' ? 'h-14 w-16 shrink-0 sm:h-20 sm:w-24' : 'h-16 w-full sm:h-28'}`}>{isMenuImage(product.imageUrl) ? <img src={product.imageUrl} alt={product.name} className="h-full w-full object-cover transition duration-300 group-hover:scale-105" /> : <span className="text-xl sm:text-4xl">{product.imageUrl || <ChefHat className="h-5 w-5 text-emerald-500 sm:h-8 sm:w-8" />}</span>}</div><div className={`min-w-0 flex-1 ${menuView === 'compact' ? 'p-2 sm:p-3' : 'p-2.5 sm:p-4'}`}><p className={`line-clamp-2 font-black ${menuView === 'compact' ? 'text-sm' : 'text-xs sm:text-sm'}`}>{product.name}</p><div className="mt-1 flex items-center justify-between gap-1.5 sm:mt-2 sm:gap-2"><span className="truncate text-[9px] font-bold uppercase text-gray-400 sm:text-[10px]">{product.menuVariants?.length ? `${product.menuVariants.length} choices` : product.category}</span><span className="shrink-0 font-mono text-[10px] font-black text-emerald-500 sm:text-sm">{product.menuVariants?.length ? 'From ' : ''}{settings.currency}{Math.min(product.sellingPrice, ...(product.menuVariants || []).map(variant => variant.price)).toFixed(2)}</span></div></div></button>)}</div> : <div className="flex min-h-64 flex-col items-center justify-center text-center text-gray-500"><ChefHat className="mb-3 h-8 w-8 opacity-40" /><p className="font-bold">No menu items found</p><p className="mt-1 text-xs">Add dishes in Menu Items or change the filter.</p></div>}
          </div>
        </section>
...
function Field({icon: Icon, label, children}: {icon: typeof Armchair; label: string; children: React.ReactNode}) { return <div className="min-w-0"><span className="mb-1.5 flex items-center gap-2 text-[10px] font-black uppercase tracking-widest text-gray-400"><Icon className="h-3.5 w-3.5" />{label}</span>{children}</div>; }

function OrderCounter({value, onChange, label}: {value: number; onChange: (value: number) => void; label: string}) {
  return <div className="flex h-11 overflow-hidden rounded-xl border border-gray-200 bg-gray-50 dark:border-white/10 dark:bg-white/[0.035]"><button type="button" onClick={() => onChange(Math.max(1, value - 1))} disabled={value <= 1} aria-label={`Decrease ${label}`} className="flex w-10 items-center justify-center border-r border-gray-200 text-gray-500 transition hover:bg-gray-100 disabled:cursor-not-allowed disabled:opacity-35 dark:border-white/10 dark:hover:bg-white/5"><Minus className="h-4 w-4" /></button><output className="flex min-w-0 flex-1 items-center justify-center text-sm font-black">{value}</output><button type="button" onClick={() => onChange(value + 1)} aria-label={`Increase ${label}`} className="flex w-10 items-center justify-center border-l border-gray-200 text-emerald-500 transition hover:bg-emerald-500/10 dark:border-white/10"><Plus className="h-4 w-4" /></button></div>;
}
