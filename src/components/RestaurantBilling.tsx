    ...
    <div className="space-y-5">
      ...
      <div className="grid gap-5 sm:grid-cols-[minmax(0,1fr)_280px] md:grid-cols-[minmax(0,1fr)_300px] lg:grid-cols-[minmax(0,1fr)_360px] xl:grid-cols-[minmax(0,1fr)_390px]">
        <section className="min-w-0 space-y-4">
          {orderType === 'Dine In' && <div className="grid grid-cols-2 gap-2 rounded-3xl border border-gray-200 bg-white p-3 dark:border-white/10 dark:bg-[#141416] sm:gap-3 sm:p-4"><Field icon={Armchair} label="Table"><input value={tableNumber} onChange={event => setTableNumber(event.target.value)} className="restaurant-input min-w-0 !px-3 !py-2.5" placeholder="Table no." /></Field><Field icon={UsersRound} label="Guests"><input type="number" min="1" value={guestCount} onChange={event => setGuestCount(Math.max(1, Number(event.target.value)))} className="restaurant-input min-w-0 !px-3 !py-2.5" /></Field></div>}
...
function Field({icon: Icon, label, children}: {icon: typeof Armchair; label: string; children: React.ReactNode}) { return <label className="min-w-0"><span className="mb-1.5 flex items-center gap-2 text-[10px] font-black uppercase tracking-widest text-gray-400"><Icon className="h-3.5 w-3.5" />{label}</span>{children}</label>; }
